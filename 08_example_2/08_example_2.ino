...
void loop() {
  ...
  if (distance >= 100.0 && distance <= 300.0) 
  {
    if (distance <= 200.0)
    {
      value = Return_Value(distance);
      analogWrite(PIN_LED,value);
    }
    else
    {
      value = Return_Value2(distance);
      analogWrite(PIN_LED,value);
    }
  }
  else
  ...
  Serial.print(",Light:");       Serial.print(value);
  Serial.println("");  
...
int Return_Value(float v)
{
  return (int)abs(((2.55 * v - 255) - 255));
}
int Return_Value2(float v)
{
  return (int)abs(-2.55 * v + 510);
}
