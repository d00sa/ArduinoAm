...
void loop() {
  ...
  if ((dist_raw == 0.0) || (dist_raw > _TARGET_HIGH)) {
    dist_raw = dist_prev;           // Cut higher than maximum
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (dist_raw < _TARGET_LOW) {
  ...
  else
  {
    ...
    if (dist_ema > 270.0)
    {
      add = 90 - (int)(_TARGET_HIGH - dist_ema);
    }
    else
    {
      add = -90 - (int)(_TARGET_LOW - dist_ema); 
    }
    myservo.writeMicroseconds(_DUTY_NEU + (_DUTY_ADD * add));
    delay(50);
  }
